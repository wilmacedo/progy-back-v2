import { makeList } from '@/use-cases/factories/perspective/make-list';
...
export async function list(request: Request, response: Response) {
  ...
  try {
    ...
    const { perspectives } = await listCase.execute({
      planningId: id,
      filter: {
        populate,
      },
    ...
    return response.status(200).json({ data: perspectives });
  } catch (error) {
